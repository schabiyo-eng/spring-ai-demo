...
/**
 * Integration tests for observation instrumentation in {@link VoyageAiEmbeddingModel}.
 *
 * @author Spring AI Onboarding
 * @since 2.0.1
 */
@SpringBootTest(classes = VoyageAiEmbeddingModelObservationIT.Config.class)
@EnabledIfEnvironmentVariable(named = "VOYAGE_AI_API_KEY", matches = ".+")
class VoyageAiEmbeddingModelObservationIT {
	...
	void observationForEmbeddingOperation() {
		var options = VoyageAiEmbeddingOptions.builder().model(VoyageAiApi.EmbeddingModel.VOYAGE_3.getValue()).build();
		...
		TestObservationRegistryAssert.assertThat(this.observationRegistry)
			...
			.that()
			.hasContextualNameEqualTo("embedding " + VoyageAiApi.EmbeddingModel.VOYAGE_3.getValue())
			.hasLowCardinalityKeyValue(LowCardinalityKeyNames.AI_OPERATION_TYPE.asString(),
					AiOperationType.EMBEDDING.value())
			.hasLowCardinalityKeyValue(LowCardinalityKeyNames.AI_PROVIDER.asString(), AiProvider.VOYAGE_AI.value())
			.hasLowCardinalityKeyValue(LowCardinalityKeyNames.REQUEST_MODEL.asString(),
					VoyageAiApi.EmbeddingModel.VOYAGE_3.getValue())
	...
	static class Config {
		...
		public VoyageAiApi voyageAiApi() {
			return VoyageAiApi.builder().apiKey(System.getenv("VOYAGE_AI_API_KEY")).build();
		}

		@Bean
		public VoyageAiEmbeddingModel voyageAiEmbeddingModel(VoyageAiApi voyageAiApi,
				TestObservationRegistry observationRegistry) {
