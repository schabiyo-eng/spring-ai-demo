...
public class VoyageAiEmbeddingModel extends AbstractEmbeddingModel {
	...
	/**
	 * Known embedding dimensions for Voyage AI models.
	 */
	private static final Map<String, Integer> KNOWN_EMBEDDING_DIMENSIONS = Map.of(
			VoyageAiApi.EmbeddingModel.VOYAGE_3.getValue(), 1024, VoyageAiApi.EmbeddingModel.VOYAGE_3_LITE.getValue(),
			512, VoyageAiApi.EmbeddingModel.VOYAGE_CODE_3.getValue(), 1024,
			VoyageAiApi.EmbeddingModel.VOYAGE_CODE_2.getValue(), 1536, VoyageAiApi.EmbeddingModel.VOYAGE_2.getValue(),
			1024, VoyageAiApi.EmbeddingModel.VOYAGE_FINANCE_2.getValue(), 1024,
			VoyageAiApi.EmbeddingModel.VOYAGE_LAW_2.getValue(), 1024);
	...
	public EmbeddingResponse call(EmbeddingRequest request) {
		...
		return EmbeddingModelObservationDocumentation.EMBEDDING_MODEL_OPERATION
			...
			.observe(() -> {
				var responseEntity = RetryUtils.execute(this.retryTemplate,
						() -> this.voyageAiApi.embeddings(apiRequest));
				VoyageAiApi.EmbeddingList<VoyageAiApi.Embedding> apiResponse = responseEntity.getBody();
	...
	private EmbeddingRequest buildEmbeddingRequest(EmbeddingRequest embeddingRequest) {
		...
		if (requestOptions != null) {
			...
			if (requestOptions instanceof VoyageAiEmbeddingOptions voyageOptions) {
				builder.inputType(
						ModelOptionsUtils.mergeOption(voyageOptions.getInputType(), this.options.getInputType()));
			}
			else {
				builder.inputType(this.options.getInputType());
			}
	...
	private DefaultUsage getDefaultUsage(VoyageAiApi.@Nullable Usage usage) {
		if (usage == null || usage.totalTokens() == null) {
			return new DefaultUsage(0, 0, 0, usage);
		}
		return new DefaultUsage(usage.totalTokens(), 0, usage.totalTokens(), usage);
	}

	private VoyageAiApi.EmbeddingRequest<List<String>> createRequest(EmbeddingRequest request) {
		VoyageAiEmbeddingOptions requestOptions = (VoyageAiEmbeddingOptions) Objects
			.requireNonNull(request.getOptions());
		return new VoyageAiApi.EmbeddingRequest<>(request.getInstructions(), requestOptions.getModel(),
				requestOptions.getInputType());
	}
